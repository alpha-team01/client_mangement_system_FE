...
import { makeCustomerPayment, uploadCustomerPaymentSlip } from "../../../api/services/Common";
...
export const Pending = ({ data }: PendingProps) => {
  ...
  useEffect(() => {
    setTableData(data.stateWisePaymentDetails.map(
      (item: any) => {
        return {
          key: item.id,
  ...
  const handleOnFinishPayment = async (values: any) => {
    ...
    try {
      ...
      const paymentSlipUrl = await uploadCustomerPaymentSlip(fileList[0], customer.key);
      ...
      const userName = `${user.firstName ?? "Admin"} ${user.lastName ?? ""}`.trim();
        ...
        <Row>
          <Col sm={10} lg={8}>
            <Form name="inProgress" layout="vertical">
              ...
              <Row gutter={[20, 0]}>
                <Col sm={10} lg={12}>
                  <Form.Item name="payment" label="Add Payment">
                    <Button
                ...
                >
                  <Form form={paymentForm} name="paymentForm" layout="vertical" onFinish={handleOnFinishPayment}>
                    <Form.Item
                      name="amount"
                      label="Amount"
                    ...
                    >
                      <Dragger {...uploadProps}>
                        ...
                        <p className="ant-upload-text">Click or drag file to this area to upload slip</p>
                        <p className="ant-upload-hint">Support PDF and Image files only. Maximum size of 2MB</p>
                      </Dragger>
                    </Form.Item>
                  </Form>
                </Modal>
